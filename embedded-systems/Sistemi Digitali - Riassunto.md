...
- Linguaggi per la **programmazione** delle FPGA
    ...
    - **HLS** (*High Level Synthesis*) → high(er)-level
        ...
        - Si usa un linguaggio di alto livello con “accorgimenti specifici”, ad esempio in C
...
- **LUT** → Look-up Table
    - Implementa funzione combinatoria
...
- **Mux** → per poter bypassare l’FFD (solo per reti combinatorie).
...
- Per generare clock a una data frequenza si usano **PLL** (*Phase-Locked Loop*) o **DCM** (*Digital Clock Manager*), che sono nient’altro che moltiplicatori/divisori
    - Generano segnali stabili a partire da un segnale esterno
...
2. Prima SDA, poi SCL vanno a 0 (start condition)
3. Master emette address frame (indirizzo slave + R/W bit)
4. Slave risponde con ACK (stop bit), ossia porta a 0 SDA
5. SDA torna a 1, e SCL va a 0 (fine prima fase)
6. Master torna ad emettere il clock
...
### **Utilizzo**
- Ottimo per comunicazioni di quantità modeste di dati
- $\small freq \in \{100KHz..1MHz\}$
- SPI è un protocollo simile, ma con 4 fili.

## AXI
Si divide in 3 protocolli (sviluppati da *ARM* ❤️):
1. **AXI** → complesso, ma veloce
2. **AXI Lite** → economico, più “lento” (no burst transfer)
3. **AXI Stream** → *addressless*.
...
Caratteristiche: ***addressless***, *master/slave*, ***handshake***, numero di trasferimenti non limitato, ***parallelismo*** configurabile.

Master e slave si sincronizzano con l’handshake.
...
| Funzioni come 
First-Class Citizens | No | Sì |
| Supporto alla 
creazione di https://en.wikipedia.org/wiki/Domain-specific_language | No | Sì |
